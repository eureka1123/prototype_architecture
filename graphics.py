...
def visualize_prototypes(x, action, img):
	screen_width = 600
	screen_height = 400
	x_threshold = 2.4
	...
	color = (255, 0, 0)  
	thickness = 4
	arrow_x = cartx+int(round(arrow_len*x[1]))
	...
	color = (0, 0, 255)  
	thickness = 4
	arrow_x = polex+int(round(arrow_len*x[3]))
	...
	color = (0,0,0)
	org = (20, 20) 
	cv2_img = cv2.putText(cv2_img, "action: "+str(action), org, font, fontScale, color, thickness, cv2.LINE_AA)
	return cv2_img
